...
class PenWidthTestSketch(vsketch.SketchClass):
    ...
    min_buffer = vsketch.Param(0.1, decimals=4, unit="in")
    ...
    def draw(self, vsk: vsketch.Vsketch) -> None:
        ...
        x_max_radius = (vsk.width - self.min_buffer *
                        (self.num_steps + 1)) / (2 * self.num_steps)
        y_max_radius = (vsk.height - (self.label_height if self.label else 0) -
                        self.min_buffer *
                        (self.num_layers + 1)) / (2 * self.num_layers)
